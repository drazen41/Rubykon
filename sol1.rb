...
class MyPiece1 < Piece
  ...
  def self.next_piece (board)
    MyPiece1.new(All_My_Pieces.sample, board)
  end
end

class MyBoard1 < Board
  # your enhancements here
  ...
  def next_piece
    if !self.willCheat
      @current_block = MyPiece1.next_piece(self)
    else
      self.willCheat = false
      @score -= 100
      @current_block = MyPiece1.new([[[0,0]]], self)
    end
    @current_pos = nil
  end
