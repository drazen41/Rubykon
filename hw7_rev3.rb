...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(x1, x2)
      aXFst, aYFst, aXEnd, aYEnd, bXFst, bYFst, bXEnd, bYEnd = y1 < seg.y1 ? seg1 + seg2 : seg2 + seg1
      if real_close(aYEnd, bYFst)
        Point.new(aXEnd, aYEnd)
      elsif aYEnd < bYFst
        NoPoints.new
      elsif aYEnd > bYEnd
        LineSegment.new(bXFst, bYFst, bXEnd, bYEnd)
      else
        LineSegment.new(bXFst, bYFst, aXEnd, aYEnd)
      end
    else
      aXFst, ayFst, aXEnd, aYEnd, bXFst, bYFst, bXEnd, bYEnd = x1 < seg.x1 ? seg1 + seg2 : seg2 + seg1
      if real_close(aXEnd, bXFst)
        Point.new(aXEnd, aYEnd)
      elsif aXEnd > bXEnd
        LineSegment.new(bXFst, bYFst, bXEnd, bYEnd)
      else
